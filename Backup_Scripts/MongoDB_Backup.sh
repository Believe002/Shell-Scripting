...
OutFile="MongoDB_Backup_$(date +%Y-%m-%d).sql";

DirFormat="MongoDB_$(date +%Y-%m-%d)";
echo -e "\nThe Formate for folder is : $DirFormat\n";
echo -e "The Format for File is : $OutFile\n";

# Perfoming Backup
mongodump --username "$User" --password "$Password" --db "$DbName" --out /var/tmp/  2>/dev/null;
...
mkdir /var/Backup/$DirFormat 2> /dev/null;
...
mv /var/tmp/$DbName /var/Backup/$DirFormat/
